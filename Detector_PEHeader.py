...
def get_PEHeader_features(path, dictVec):
    PEHeader_features = {}
    try:
        pe = pefile.PE(path)
        pe.parse_data_directories()
        for i in range(len(pe.OPTIONAL_HEADER.__keys__)):
            optional = pe.OPTIONAL_HEADER.__keys__[i][0]
            PEHeader_features[optional] = pe.OPTIONAL_HEADER.__getattribute__(optional)           
    except:
        print("Unable to parse {0}".format(path))
        PEHeader_features = dict(zip(range(30), [0]*30))
    
    print("Extracted {0} PE Headers from {1}".format(len(PEHeader_features),path))
    dictVec_features = dictVec.fit_transform([PEHeader_features])
    dictVec_features = dictVec_features.todense()
    dictVec_features = numpy.asarray(dictVec_features)
    dictVec_features = dictVec_features[0]
    return dictVec_features
...
def scan_file(path):
    ...
    with open("saved_detector_PEHeader.pkl","rb") as saved_detector:
        classifier, dictVec = pickle.load(saved_detector)
    features = get_PEHeader_features(path,dictVec)
...
def train_detector(benign_path,malicious_path,dictVec):
    ...
    X = [get_PEHeader_features(path,dictVec) for path in malicious_paths + benign_paths]
    ...
    pickle.dump((classifier,dictVec),open("saved_detector_PEHeader.pkl","wb+"))
    print("End Saving Models...")

def cv_evaluate(X,y,dictVec):
...
def get_training_data(benign_path,malicious_path,dictVec):
    def get_training_paths(directory):
        targets = []
        for path in os.listdir(directory):
            targets.append(os.path.join(directory,path))
...
def main():
    ...
    parser.add_argument("--evaluate",default=True,action="store_true",help="Perform cross-validation")

    args = parser.parse_args()

    dictVec = DictVectorizer(dtype=numpy.float64)
    ...
    if args.scan_file_path:
        ...
        train_detector(args.benignware_paths,args.malware_paths,dictVec)
    elif args.malware_paths and args.benignware_paths and args.evaluate:
        X, y = get_training_data(args.benignware_paths,args.malware_paths,dictVec)
        cv_evaluate(X,y,dictVec)
    else:
